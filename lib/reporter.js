(function(request, Joi, Promise, util, lodash, logger) {
    ...
    var accountTokenValidation = Joi.string().required().min(1).example('1234567890'); //TODO: Revisit this validation to see if we can better validate the values
    var usernameValidation = Joi.string().required().min(1).example('1234567890'); //TODO: Revisit this validation to see if we can better validate the values
    var projectNameValidation = Joi.string().required().min(1).example('1234567890'); //TODO: Revisit this validation to see if we can better validate the values
    ...
    function sendForLanguage(endpoint, commitId, language, token, accountToken, username, projectName, data) {
        if (accountToken) {
            return sendLanguageByAccount(endpoint, commitId, language, accountToken, username, projectName, data)
                .then(function(erhy ) {
                    if (err) {
                        return Promise.reject(err);
                    }
    ...
    function sendLanguageByAccount(endpoint, commitId, language, accountToken, username, projectName, data) {
        ...
        return {
            sendCoverage: function sendCoverage(token, commitId, language, data, accountToken, username, projectName) {
                ...
                if (accountToken) {
                    ...
                    validationErr = validationErr || accountTokenValid || usernameValid || projectNameValid;
                } else {
